package cn.jiming.jdlearning.obj.train;
 ...
 * @author xiaoming
 */
public class ObjClassificationTrain {
...
//	private static ExecutorService pool = Executors.newFixedThreadPool(20);
	 ...
	 */
	public static void training(String model, String trainPath, String objNetPath) {
		ObjNet objNet = null;
		if(model == null) {
			objNet = new ObjNet();
		}else {
			objNet = ObjNet.load(model);
		}
		
		dataSet(trainPath, objNet);
...
//		}
		...
		//保存网络模型
		objNet.saveNet(objNetPath);
	}



	private static void dataSet(String trainPath, ObjNet objNet) {
		int i = 0;
...
//						});
				...
				new TrainObjClassificationException("Path:" + trainPath + " is not empty.");
			}
		}else {
			new TrainObjClassificationException("Path:" + trainPath + " is not data directory.");
		}
	}



	private static void loadData(ObjNet objNet, File lab, String labName) {
		...
		if(faces != null && faces.length > 0) {
			for(File faceF:faces) {
				...
				try {
					...
					objNet.addFeature(labName, feature);
				} catch (Exception e) {
					e.printStackTrace();
				}
